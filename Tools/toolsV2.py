...
class ProbabilisticMetrics:
    ...
    def get_kl_divergence(real_label, predicted_label):
        ...
        predicted_label = np.asarray(predicted_label, dtype=np.float32)
        #print(np.sum(tf.keras.losses.KLD(real_label,predicted_label)))
        #exit()
        # Ensure they are valid probability distributions
        #size=real_label.shape
                  
        real_label /= np.sum(real_label)
        predicted_label /= np.sum(predicted_label)

        # Add a small epsilon to avoid log(0)
        epsilon = 1e-10
        real_label += epsilon
        predicted_label += epsilon

        return np.sum(rel_entr(real_label, predicted_label))
