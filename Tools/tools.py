...
DEFAULT_PLOT_REGRESSION_METRICS_LABELS = [ 'Similaridade de Cossenos','Erro Médio Quadrática','Máxima Discrepância Média']
...
class PlotClassificationMetrics:
    ...
    def plot_classifier_metrics(self, classifier_type, accuracy_list,precision_list,recall_list, f1_score_list,plot_filename, plot_title,type_of_ts):

        list_all_metrics = [accuracy_list,precision_list,recall_list, f1_score_list]

        new_plot_bars = go.Figure()
        clor_map=['#1f77b4', '#ff7f0e','#2ca02c', '#d62728']
        metrics_name=[]
        if type_of_ts =="TRTS":     
            metrics_name= ['Acurácia-Treinado com real','Precisão-Treinado com real','Recall-Treinado com real','F1-Score-Treinado com real']
        else:
            metrics_name= ['Acurácia-Treinado com sint','Precisão-Treinado com sint' ,'Recall-Treinado com sint','F1-Score-Treinado com sint']

        for metric, metric_values, color in zip(metrics_name, list_all_metrics, clor_map):
            try:
...
class P_values_metrics:
    def __init__(self, labels_plot_regressive_metrics=None, color_map_bar=None, width_bar=DEFAULT_WIDTH_BAR,
        ...
        self.font_size = font_size
    def p_value_plot(self,classifier_type,acc,prec,f1,recall,auc,plot_filename, plot_title):
        list_all_metrics = [acc,prec,recall, f1,auc]
        new_plot_bars = go.Figure()
        clor_map=['#1f77b4', '#ff7f0e','#2ca02c', '#d62728','#9467bd','#8c564b']
        metrics_name=["p_value acurácia","p_value precisão","p_value recall","p_value f1 score","p_value_auc"]
        for metric, metric_values, color in zip(metrics_name, list_all_metrics, clor_map):
            try:
                metric_mean = np.mean(metric_values)
                metric_std = np.std(metric_values)

                new_plot_bars.add_trace(go.Bar(x=[metric], y=[metric_values], name=metric, marker=dict(color=color),
                                               error_y=dict(type='constant', value=metric_std, visible=True),
                                               width=self.width_bar))
            except Exception as e:
        ...
        y_label_dictionary = dict(title=f'Valor estatístico de p', tickmode='linear', tick0=0.0, dtick=0.1,
                                  gridcolor='black', gridwidth=.05)
...
class ProbabilisticMetrics:
    ...
    def get_kl_divergence(real_label, predicted_label):
        ...
        r=(rel_entr(real_label, predicted_label))
        for i in r:
          print(i)
        exit()
    ...
    @staticmethod
    def get_log_likehood(real_label,predicted_label):
